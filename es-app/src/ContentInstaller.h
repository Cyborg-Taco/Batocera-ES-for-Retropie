...
public:
	...
	struct ContentTask
	{
		ContentTask(ContentType taskType = CONTENT_THEME_INSTALL, const std::string& taskKey = "", const std::string& taskName = "", const PacmanPackage& taskPackage = PacmanPackage())
			: type(taskType), key(taskKey), name(taskName), package(taskPackage)
		{
		}
	...
	static void Enqueue(Window* window, ContentType type, const PacmanPackage& package);
	static bool IsInQueue(ContentType type, const std::string contentName);
	static bool IsInQueue(ContentType type, const PacmanPackage& package);
	static bool isRunning() { return mInstance != nullptr; }

private: // Methods
	ContentInstaller(Window* window);
	~ContentInstaller();
	static void Enqueue(Window* window, ContentType type, const ContentTask& task);
...
private:
	...
	static std::list<ContentTask>				  mQueue;
	static std::list<ContentTask>				  mProcessingQueue;
