...
function _update_hook_Batocera-ES-for-Retropie() {
    # make sure the input configuration scripts, savestate config, and launch script are always up to date
...
function _remove_stock_emulationstation() {
    local setup_dir="$home/RetroPie-Setup"
    local setup_script="$setup_dir/retropie_setup.sh"

    if [[ -x "$setup_script" ]]; then
        pushd "$setup_dir" >/dev/null || return 1
        sudo ./retropie_setup.sh emulationstation remove
        popd >/dev/null || return 1
    else
        printMsgs "dialog" "RetroPie-Setup was not found at $setup_dir. Install/remove the default EmulationStation manually before continuing."
        return 1
    fi
}
...
function configure_Batocera-ES-for-Retropie() {
    ...
    _remove_stock_emulationstation || return 1
